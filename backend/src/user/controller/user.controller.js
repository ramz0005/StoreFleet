...
export const createNewUser = async (req, res, next) => {
  ...
  try {
    ...
    if (err.code === 11000 && err.keyValue.email) {
      return next(new ErrorHandler(400, "Email address already registered. Please use a different email."));
    }
    return next(new ErrorHandler(400, err.message || "Registration failed"));
  }
};
...
export const updateUserProfileAndRole = async (req, res, next) => {
  ...
  try {
    const updatedUserDetails = await updateUserRoleAndProfileRepo({_id: req.params.id}, {
      name,
      email,
      role
    });
